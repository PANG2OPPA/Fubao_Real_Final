...
public class MenuListDao {
    ...
    public void UserChange(List<FubaoUserVo> list) {
        ...
        switch (num) {
            case 1:
                fubaoUserDao.userChange(list); //회원정보 변경
                u_list=fubaoUserDao.Change_id(list.get(0).getId());
    ...
    public void MovieChoice(String nick, int s1_num, int v_num) {
        ...
        if (!num.equalsIgnoreCase("x")) {
            ...
            while (true) {
                System.out.println("[1]영화 재생  [2]리뷰 보기 [3]좋아요,싫어요 [4]종료");
                System.out.print("입력:");
                int num2 = sc.nextInt();
                switch (num2) {
                    case 1:
